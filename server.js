...
app.post('/api/gemini', async (req, res) => {
  ...
  try {
    if (action === 'translate') {
      ...
      const { bodyPart, round1Answers, round2Answers, lang, gender, age, condition, pain, since, medications, allergies } = req.body;
      if (!bodyPart || !lang) return res.status(400).json({ error: 'Missing bodyPart or lang' });
